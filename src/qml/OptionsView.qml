...
Page {
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            GroupBox {
                title: "Video Player" + (g_backends.length === 1 ? " (" + playerOption.currentName + ")" : "")
                padding: 10
                Layout.fillWidth: true
                Layout.maximumWidth: 500
                Layout.alignment: Qt.AlignCenter
                ...
                Column {
                    ...
                    OptionCombo {
                        ...
                        Component.onCompleted: {
                            model = g_backends.map(function(v) { return backendNames[v] || v; })
                            selectItem(Settings.backend)
                            visible = g_backends.length > 1
                        }
                        onActivated: {
                            if (Settings.backend !== g_backends[currentIndex]) {
                                Settings.backend = g_backends[currentIndex]
                            }
                        }
                        function selectItem(name) {
                            for (var i in g_backends) {
